...
Features chunked processing for handling large files efficiently during benchmarks.
"""
...
try:
    ...
    # If not available from cryptography, define a generic one or just catch Exception
    InvalidSignature = Exception # This allows the code to run if the import fails


def create_test_file(size_kb, filename="test_file.tmp", auto_delete=True):
    """
    ...
    Args:
        ...
        auto_delete (bool): Whether to automatically delete the file after creation (for internal use)
    ...
    # Note: The file is created, but auto-deletion logic is handled by caller
    return filename
...
def benchmark_encryption(file_sizes_kb, trials=3):
    ...
    for size in file_sizes_kb:
        ...
        for _ in range(trials):
            ...
            # Benchmark encryption (using chunked processing from aes_gcm.py)
            start_time = time.perf_counter()
            encrypt_file_aes(test_file, key, nonce, encrypted_file)
            encrypt_time = time.perf_counter() - start_time
            total_encrypt_time += encrypt_time

            # Benchmark decryption (using chunked processing from aes_gcm.py)
            start_time = time.perf_counter()
            decrypt_file_aes(encrypted_file, key, nonce, decrypted_file)
    ...
    # RSA Benchmarking (Limited to smaller sizes due to RSA constraints)
    ...
    for _ in range(trials * 10):  # More trials for ECC as it's fast
        ...
        start_time = time.perf_counter()
        try:
            # Use specific exception if available
...
def benchmark_hashing(file_sizes_kb, trials=3):
    """
    Benchmark SHA-256 hashing operations with chunked processing.
    ...
    for size in file_sizes_kb:
        ...
        for _ in range(trials):
            ...
            # Benchmark file hashing (using chunked processing from sha256_hash.py)
            start_time = time.perf_counter()
            file_hash = hash_file_sha256(test_file)
            file_hash_time = time.perf_counter() - start_time
            ...
            text_hash_time = time.perf_counter() - start_time
