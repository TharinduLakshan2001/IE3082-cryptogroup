...
def integrated_decryption_benchmark(input_file):
    ...
    try:
        ...
        if not os.path.exists(encrypted_file) or not os.path.exists(private_key_file) or not os.path.exists(encrypted_key_file):
            ...
            # --- FIX: Split the decrypted blob into key and nonce ---
            # Determine the original AES key size used during ini en.
            # You need to deduce this from the AES part of the hybrid encryption.
            # The ini en saved the AES-encrypted file. The total hybrid output size was:
            # len(encrypted_file_content) + len(encrypted_key_blob).
            # The AES-GCM tag is 16 bytes.
            # The AES nonce is 12 bytes.
            # So, if the original file size was F, and AES key size was K:
            # Size of AES-encrypted part in hybrid = F + 16 (tag)
            # Size of RSA-encrypted key+nonce = 36 (for 24-byte key + 12-byte nonce) or 28 (for 16+12) or 44 (for 32+12)
            # You can calculate the key size based on the length of the decrypted blob.
            # For example, if decrypted_key_nonce is 36 bytes, key size was 24, nonce was 12.
            # If it's 28, key size was 16. If it's 44, key size was 32.
            # Let's assume for now the key size was 24 (as in the example ini en command).
            # A more robust way is to save the key size used during ini en in the results directory.
            # For now, calculate based on the length of the decrypted blob.
            total_decrypted_len = len(decrypted_key_nonce)
            if total_decrypted_len < 12:
                 print_error(f"Decrypted key/nonce blob is too short: {total_decrypted_len} bytes.")
                 raise ValueError("Invalid decrypted key/nonce length.")
            # Assume nonce is always 12 bytes for GCM.
            nonce_len = 12
            key_len = total_decrypted_len - nonce_len
            if key_len not in [16, 24, 32]:
                 print_error(f"Decrypted key length {key_len} is not standard (16, 24, 32).")
                 raise ValueError("Invalid AES key length derived from decrypted blob.")

            session_key = decrypted_key_nonce[:key_len]
            session_nonce = decrypted_key_nonce[key_len: key_len + nonce_len]
            # --- END FIX ---
            # OLD LINE: session_key = decrypted_key_nonce[:len(decrypted_key_nonce)//2]
            # OLD LINE: session_nonce = decrypted_key_nonce[len(decrypted_key_nonce)//2:]
            ...
            from cryptography.hazmat.backends import default_backend
            cipher = Cipher(
                algorithms.AES(session_key), # Use the extracted session_key
                modes.GCM(session_nonce),   # Use the extracted session_nonce
                backend=default_backend()
            )
            ...
            benchmark_results['algorithms']['RSA-3072-Dec'] = {
                ...
                'key_size': 3072,  # bits for RSA
                'ciphertext_size': os.path.getsize(encrypted_file),
                'security_strength': f'3072-bit (RSA) + {len(session_key)*8}-bit (AES)', # Use len(session_key)
                'scalability': 'Hybrid (AES for data, RSA for key)'
            }
    ...
    try:
        ...
        if not os.path.exists(message_file) or not os.path.exists(signature_file) or not os.path.exists(public_key_file):
            ...
            # Load public key (FIX: Use PEM loader)
            from cryptography.hazmat.primitives import serialization
            from cryptography.hazmat.backends import default_backend
            from cryptography.hazmat.primitives.asymmetric import ed25519
            with open(public_key_file, 'rb') as f:
                 # Use the standard PEM loader instead of from_public_bytes
                 public_key = serialization.load_pem_public_key(f.read(), backend=default_backend())
            # --- END FIX ---
            ...
            # Time verification
            start_time = time.perf_counter()
            is_valid = ed25519_verify(message, signature, public_key) # Use the loaded public key object
            end_time = time.perf_counter()
            ...
            benchmark_results['algorithms']['ECC-Ed25519-Ver'] = {
                ...
                'memory_usage': memory_usage_mb,
                'key_generation_time': 0, # Key already loaded
                'key_size': 32,  # bytes for Ed25519 public key (loaded via PEM)
                'signature_size': len(signature),
                'security_strength': '128-bit equivalent',
                'scalability': 'Linear'
            }
